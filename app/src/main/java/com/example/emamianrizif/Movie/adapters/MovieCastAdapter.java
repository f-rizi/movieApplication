 ...
 */
public class MovieCastAdapter extends MovieCreditsAdapter {

    public MovieCastAdapter(List<? extends Career> movieCastList) {
        super(movieCastList);
    }

    @Override
    public void bindView(final ItemViewHolder holder, int position) {
        final MovieCast movieCast = (MovieCast) movieCreditsList.get(position);
        ...
        holder.nameTextView.setText(movieCast.getName());
        holder.characterTextView.setText(movieCast.getCharacter());

        String url = NetworkConstants.POSTER_PATH_URL + movieCast.getProfilePath();
...
//                NetworkConstants.POSTER_PATH_URL + movieCast.getProfilePath(),
//                imageLoader);

        itemHeight = holder.personLayout.getHeight();
    }
}
