 ...
 */
public class PersonCrewAdapter extends MovieCreditsAdapter {

    public PersonCrewAdapter(List<? extends Career> movieCastList) {
        super(movieCastList);
    }

    @Override
    public void bindView(final ItemViewHolder holder, int position) {
        final PersonCrew personCrew = (PersonCrew) movieCreditsList.get(position);
        ...
        holder.personLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                listener.onCreditClick(
                        personCrew.getId(),
                        personCrew.getTitle(),
                        personCrew.getPoster_path());
            }
        });

        holder.nameTextView.setText(personCrew.getDepartment());
        holder.characterTextView.setText(personCrew.getJob());

        String url = NetworkConstants.POSTER_PATH_URL + personCrew.getPoster_path();
...
//                NetworkConstants.POSTER_PATH_URL + personCrew.getPoster_path(),
//                imageLoader);
    }
}
